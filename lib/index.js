...
function plainOptions(config) {
  const read = (value) => typeof value === "object" && value !== null && VOLATILE_WRITE in value ? value.get() : value;
  const out = {};
...
function resolveAdapterOptions(options, environment) {
  const named = options.baseURL !== void 0 && options.baseURL.trim().length > 0 ? options.baseURL : environment?.get(BASE_URL_ENV)?.value;
  ...
  if (options.defaultContextWindow !== void 0 && (!Number.isInteger(options.defaultContextWindow) || options.defaultContextWindow <= 0)) {
